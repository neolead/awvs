...
RUN curl -s -o /tmp/awvs_latest.sh https://github/awvs_latest.sh
RUN cd /tmp && \
	chmod +x awvs_latest.sh
RUN curl -s -o /tmp/install.expect https://github/install.expect
RUN cd /tmp && chmod +x /tmp/install.expect && expect /tmp/install.expect
#破解awvs，并开机自启。
RUN curl -s -o awvs_crack.zip https://github/awvs_crack.zip
RUN unzip -o /awvs/awvs_listen.zip -d /home/acunetix/.acunetix/data/license/
